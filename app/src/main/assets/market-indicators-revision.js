...
/* REV20.42 — resilient independent market indicators.
   No network request on app start. Runtime owns the sole 30-minute automatic cadence; this module keeps manual refresh available.
...
(()=>{
  ...
  globalThis.refreshAurumDataMarketStrip=async function(ev){const btn=ev?.currentTarget||document.querySelector('.aurum-r209-market-refresh');if(btn?.dataset.busy==='1')return false;try{if(btn){btn.dataset.busy='1';btn.disabled=true}globalThis.showAurumNotice?.('Piyasa yenileme Veriler merkez zincirinden başlatıldı.','info',1800);return await globalThis.runDataRefresh?.('FULL')}catch(e){globalThis.showAurumNotice?.('Piyasa bilgileri alınamadı: '+(e?.message||e),'error',2400);return false}finally{const b=document.querySelector('.aurum-r209-market-refresh');if(b){delete b.dataset.busy;b.disabled=false}}};
