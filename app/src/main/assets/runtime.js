...
kh117SSeatCount=function kh117SSeatCountR225(k){const top=new Set((S.scores?.[k]||[]).slice(0,Math.max(20,Number(S.settings?.topN||20))).map(x=>String(x?.sym||'')).filter(Boolean)),selected=new Set((S.selection||[]).map(x=>String(x?.sym||'')).filter(Boolean));let n=0;for(const sym of top)if(selected.has(sym))n++;return n;};globalThis.kh117SSeatCount=kh117SSeatCount;
