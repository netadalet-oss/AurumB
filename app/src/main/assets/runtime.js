...
async function prepareGeneralData(job,mode='GENERAL'){
  ...
  try{
    ...
    const end=new Date(),start=addMonths(end,-Number(state.settings.monthsBack||14)),indicatorPromise=refreshMarketIndicators({auto:true,force:String(job?.mode||'').toUpperCase()==='MANUAL'});let indexBundle=(await dbGet('meta','indexBundle'))?.value||{bars:[]};
...
function kn117MarketTime(rec){if(rec?.marketTimeVerified===true&&rec?.marketDataAt&&Number.isFinite(Date.parse(rec.marketDataAt)))return rec.marketDataAt;const z=rec?.series||{};for(const key of ['marketTime','timestamp','time','at']){const x=z?.[key];if(Array.isArray(x))for(let i=x.length-1;i>=0;i--){const v=x[i];if(v&&Number.isFinite(Date.parse(v)))return v;}}const p=rec?.provenance;if(p?.marketTimeVerified===true&&p?.marketAt&&Number.isFinite(Date.parse(p.marketAt)))return p.marketAt;const held=rec?.lastVisibleMarketAt;if(held&&Number.isFinite(Date.parse(held)))return held;return TABLE_META?.data?.market&&Number.isFinite(Date.parse(TABLE_META.data.market))?TABLE_META.data.market:null;}
...
(function(){
  ...
  (function installR47MarketSummary(){
    ...
    globalThis.marketPage=function marketPageR47(){const cached=load();return `<div class="actions" style="margin-bottom:12px"><button class="ghost-btn" type="button" onclick="goPage('overview')">← Genel Bakışa Dön</button><button class="gold-btn" type="button" onclick="AurumMarketModuleRefresh()">Piyasayı Yenile</button></div><div class="section-head"><div class="section-title"><h2>Piyasa Özeti</h2></div><small>Haber · şirket · döviz · altın · borsa</small></div><div id="aurumMarketSummaryBody">${cached?render(cached):'<div class="card"><p class="muted">Piyasa özeti yükleniyor…</p></div>'}</div>`;};
  })();
...
(function installAurumRMarketIntelV2(){
 ...
 const base=globalThis.marketPage||marketPage;globalThis.marketPage=marketPage=function marketPageIntelV2(){const old=base(),c=load();return old+`<div class="section-head"><div class="section-title"><h2>Genişletilmiş Piyasa İstihbaratı</h2></div><button class="ghost-btn" onclick="AurumMarketModuleRefresh()">Piyasayı Yenile</button></div><div id="aurumRMarketIntel">${c?render(c):'<div class="card">Açık kaynaklar taranıyor…</div>'}</div>`};
})();
...
(()=>{
  ...
  function r221SuppressOutOfCohortLive(r,job){
    if(!r)return r;const x=JSON.parse(JSON.stringify(r));x.lastVisiblePrice=(Number.isFinite(Number(x.livePrice))&&Number(x.livePrice)>0)?Number(x.livePrice):(Number.isFinite(Number(x.lastVisiblePrice))?Number(x.lastVisiblePrice):null);x.lastVisibleMarketAt=x.marketDataAt||x.provenance?.marketAt||x.lastVisibleMarketAt||null;x.lastVisibleMarketProvider=x.marketTimeProvider||x.provenance?.marketTimeProvider||x.lastVisibleMarketProvider||null;x.livePrice=null;x.dayChange=null;x.liveAt=null;x.marketDataAt=null;x.marketTimeVerified=false;x.marketTimeProvider=null;x.marketWindowEligible=false;x.marketWindowDeltaMinutes=null;x.calculationEligible=false;x.r221CohortLiveSuppressed=true;x.dataIssues=[...new Set([...(x.dataIssues||[]),'OUTSIDE_30M_COHORT_LIVE_SUPPRESSED'])];x.calculationExclusionReasons=[...new Set([...(x.calculationExclusionReasons||[]),'OUTSIDE_30M_COHORT'])];x.provenance={...(x.provenance||{}),marketAt:null,marketTimeVerified:false,marketTimeProvider:null,canonicalMarketAt:job.canonicalMarketAt||null};x.unresolvedFields=[...new Set([...(bundleRecordMissingFields(x)||[]),'Anlik','VeriZamani','FiyatDegisim%_T0'])];return x
  }
...
(function installR207FinancePortal(){
  ...
  function render(data){
    ...
    return `<div class="r207-portal-head"><div><small>BİRLEŞTİRİLMİŞ CANLI FİNANS VE GÜNDEM PORTALI</small><h2>Piyasa · Finans · Yorum · Ekonomi · Gündem · Dünya</h2><p>Öncelik: güncel ve son dakika piyasa/finans → yorum ve öneriler → ekonomi → siyaset → asayiş/adalet → dış politika → dünya → içerikli şirket/KAP. Şablon ve içeriksiz kartlar gösterilmez.</p></div><button class="ghost-btn" onclick="AurumMarketModuleRefresh()">Piyasayı Yenile</button></div><div class="r207-news-chips">${cats.map(btn).join('')}</div>${lead?`<div class="r207-featured">${card(lead,true)}</div>`:'<div class="card notice">Kaynaklı ve yeterli içerik taşıyan güncel haber bulunamadı. İçeriksiz şablon kartlar gösterilmiyor.</div>'}<div class="r207-news-grid">${rest.map(x=>card(x,false)).join('')}</div><div class="r207-news-foot"><span>Kaynaklar: Google News açık RSS · Bigpara · İş Yatırım · TCMB · Borsa İstanbul</span><span>${sourceTotal?`Kaynak yanıtı ${sourceOk}/${sourceTotal} · `:''}${data?.updatedAt?'Akış yenileme: '+esc(new Date(data.updatedAt).toLocaleString('tr-TR')):''}</span></div>`
  }
...
(function installR222Contract(){
  ...
  globalThis.marketPage=function marketPageR222Unified(){
    ...
    return `<div class="actions r222-market-actions"><button class="ghost-btn" type="button" onclick="goPage('overview')">← Genel Bakışa Dön</button><button class="gold-btn" type="button" onclick="AurumMarketModuleRefresh()">Piyasayı Yenile</button></div><div class="section-head r222-market-head"><div class="section-title"><h2>Piyasa Özeti</h2></div><small>Doğrudan piyasa verisi · kaynaklı haber akışı</small></div>${typeof globalThis.marketIndicatorsMarkup==='function'?globalThis.marketIndicatorsMarkup():''}<section class="r207-portal-wrap r222-unified-portal"><div id="aurumFinancePortal"><div class="card notice"><b>Birleştirilmiş finans portalı hazırlanıyor…</b><p class="muted">Piyasa, ekonomi, şirket ve küresel kaynaklar eşzamanlı taranıyor.</p></div></div></section>`;
  };
...
(()=>{
  ...
  if(typeof base==='function')globalThis.marketPage=function marketPageR243Final(){
    ...
    queueMicrotask(async()=>{
      ...
      try{
        /* UI recovery is cache-only; navigation never starts market network work. */
        restore();save();
        const portal=document.getElementById('aurumFinancePortal');
        if(portal&&!validPortalDom(portal)&&portalCached()){
...
/* R225 — display continuity + K_Tarihsel live mirror + daily notification journal. */
(function installAurumR225Continuity(){
if(globalThis.AURUM_R225_CONTINUITY==='R225.0')return;globalThis.AURUM_R225_CONTINUITY='R225.0';
const S=globalThis.AurumUpdateAPI?.state||state;
const dayKey=()=>new Intl.DateTimeFormat('en-CA',{timeZone:'Europe/Istanbul',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date());
const sourceSeriesTime=rec=>{const z=rec?.series||{},n=Array.isArray(z.date)?z.date.length:0;for(const key of ['marketTime','timestamp','time','at']){const a=z?.[key];if(Array.isArray(a)&&n){const v=a[n-1];if(v&&Number.isFinite(Date.parse(v)))return v;}}return null;};
const latestClose=rec=>{const a=rec?.series?.calcClose||rec?.series?.close||[];for(let i=(a?.length||0)-1;i>=0;i--){const v=Number(a[i]);if(Number.isFinite(v)&&v>0)return v;}const v=Number(rec?.price);return Number.isFinite(v)&&v>0?v:null;};
function visiblePrice(rec){let open=true;try{const p=trParts();open=!!p&&p.wd!=='Sat'&&p.wd!=='Sun'&&!globalThis.schedulerIsHolidayDate?.(p.date)&&p.min>=600&&p.min<=1080;}catch{}if(open){const live=Number(rec?.livePrice);if(Number.isFinite(live)&&live>0)return live;const held=Number(rec?.lastVisiblePrice);if(Number.isFinite(held)&&held>0)return held;}return latestClose(rec)??(Number.isFinite(Number(rec?.lastVisiblePrice))&&Number(rec.lastVisiblePrice)>0?Number(rec.lastVisiblePrice):null);}
function visibleMarketTime(rec){if(rec?.marketTimeVerified===true&&rec?.marketDataAt&&Number.isFinite(Date.parse(rec.marketDataAt)))return rec.marketDataAt;const held=rec?.lastVisibleMarketAt;if(held&&Number.isFinite(Date.parse(held)))return held;const src=sourceSeriesTime(rec);if(src)return src;const p=rec?.provenance;if(p?.marketTimeVerified===true&&p?.marketAt&&Number.isFinite(Date.parse(p.marketAt)))return p.marketAt;return null;}
const rawBase=v141225Raw;v141225Raw=function v141225RawR225(rec,key,skipCompleteness=false){if(key==='Anlik')return visiblePrice(rec);if(key==='VeriZamani')return visibleMarketTime(rec);return rawBase(rec,key,skipCompleteness);};globalThis.v141225Raw=v141225Raw;
kn117LatestPrice=function kn117LatestPriceR225(rec){return visiblePrice(rec);};kn117MarketTime=function kn117MarketTimeR225(rec){return visibleMarketTime(rec);};globalThis.kn117LatestPrice=kn117LatestPrice;globalThis.kn117MarketTime=kn117MarketTime;
const rowsBase=kh117Rows;kh117Rows=function kh117RowsR225(){const current=kh117T0(),a=kh117ArchiveState();let old=[];try{old=rowsBase()||[]}catch{}const archived=old.filter(x=>!x?._t0&&String(x?.date||'')!==String(current?.date||'')).slice(0,30),fallback=kh117NormalizeArchiveRows(a?.rows||[],current?.date).slice(0,30),tail=archived.length?archived:fallback.map((r,i)=>({...kh117CloneValue(r),_label:'T'+(i+1),_t0:false,provisional:false,frozen:true}));return [{...current,_label:'T0',_t0:true,_cleared:false,_awaiting:false},...tail.map((r,i)=>({...r,_label:'T'+(i+1),_t0:false}))];};globalThis.kh117Rows=kh117Rows;try{globalThis.AurumKnHistoryV117=Object.freeze({...globalThis.AurumKnHistoryV117,rows:kh117Rows});}catch{}
kh117SSeatCount=function kh117SSeatCountR225(k){const top=new Set((S.scores?.[k]||[]).slice(0,20).map(x=>String(x?.sym||'')).filter(Boolean)),selected=new Set((S.selection||[]).map(x=>String(x?.sym||'')).filter(Boolean));let n=0;for(const sym of top)if(selected.has(sym))n++;return n;};globalThis.kh117SSeatCount=kh117SSeatCount;
if(!document.getElementById('aurumR225ContinuityCss')){const st=document.createElement('style');st.id='aurumR225ContinuityCss';st.textContent='.kh-trend{min-width:0;overflow:hidden}.kh-trend footer{min-width:0;overflow:hidden}.kh-trend footer small:last-child{display:block;max-width:100%;min-width:0;font-size:6px!important;line-height:1.05!important;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;letter-spacing:-.02em}.aurum-r225-notify-btn{border:1px solid rgba(243,211,111,.28);background:rgba(7,19,39,.78);color:var(--gold2);border-radius:9px;min-width:32px;height:30px;padding:0 8px;cursor:pointer}.aurum-r225-log{display:grid;gap:5px;max-height:52vh;overflow:auto}.aurum-r225-log-row{border-bottom:1px solid rgba(255,255,255,.07);padding:6px 2px}.aurum-r225-log-row small{display:block;opacity:.72;margin-top:2px;word-break:break-word}';document.head.appendChild(st);}
const journal={rows:[],day:null};function sameDay(ts,d=dayKey()){try{return new Intl.DateTimeFormat('en-CA',{timeZone:'Europe/Istanbul',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date(ts))===d}catch{return false}}
function journalMarkup(){const d=journal.day||dayKey(),rows=(journal.rows||[]).filter(x=>sameDay(x.ts||x.at,d)).sort((a,b)=>Date.parse(b.ts||0)-Date.parse(a.ts||0));return '<details class="card gold-edge aurum-settings-details" id="aurumR225Notifications"><summary class="aurum-settings-summary"><div><strong>Bildirimler</strong><small>'+html(d)+' 00:00 → ertesi gün 00:00 · sistem, hesaplama ve bildirim günlüğü</small></div><span class="aurum-details-chevron" aria-hidden="true">⌄</span></summary><div class="aurum-settings-details-body"><div class="actions"><button class="ghost-btn" type="button" onclick="AurumNotifications.refresh()">Yenile</button></div><div id="aurumR225Log" class="aurum-r225-log">'+(rows.length?rows.map(x=>'<div class="aurum-r225-log-row"><b>'+html(String(x.level||'info').toUpperCase())+' · '+html(x.message||'')+'</b><small>'+html(formatTableTime(x.ts||x.at))+(x.meta&&Object.keys(x.meta).length?' · '+html(JSON.stringify(x.meta)):'')+'</small></div>').join(''):'<div class="muted">Bugün için kayıt yok.</div>')+'</div></div></details>';}
async function refreshJournal(){journal.day=dayKey();let rows=[];try{rows=await dbAll('logs')}catch{}journal.rows=(rows||[]).filter(x=>sameDay(x.ts||x.at,journal.day));const host=document.getElementById('aurumR225Log');if(host){const wrap=document.createElement('div');wrap.innerHTML=journalMarkup();const next=wrap.querySelector('#aurumR225Log');if(next)host.innerHTML=next.innerHTML;}return journal.rows;}
const settingsBase=globalThis.settingsPage||settingsPage;if(typeof settingsBase==='function'){globalThis.settingsPage=settingsPage=function settingsPageR225(){queueMicrotask(()=>refreshJournal().catch(()=>{}));return journalMarkup()+settingsBase();};globalThis.settingsSub=globalThis.settingsPage;}
function openJournal(){try{if(typeof goPage==='function')goPage('settings');else{S.page='settings';render();}}catch{S.page='settings';try{render()}catch{}}setTimeout(()=>{const d=document.getElementById('aurumR225Notifications');if(d){d.open=true;d.scrollIntoView({behavior:'smooth',block:'start'});}refreshJournal().catch(()=>{});},40);}
function installBell(){const host=document.querySelector('.top-actions');if(!host||document.getElementById('aurumR225NotifyBtn'))return;const b=document.createElement('button');b.id='aurumR225NotifyBtn';b.type='button';b.className='aurum-r225-notify-btn';b.title='Bildirimler ve günlük';b.setAttribute('aria-label','Bildirimler ve günlük');b.textContent='●';b.onclick=openJournal;host.prepend(b);}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',installBell,{once:true});else installBell();
const baseLog=log;log=async function logR225(level,message,meta={}){const rec=await baseLog(level,message,meta);if(sameDay(rec.ts||rec.at)){journal.rows.unshift(rec);journal.rows=journal.rows.slice(0,5000);}return rec;};globalThis.log=log;
const noticeBase=showAurumNotice;showAurumNotice=function showAurumNoticeR225(message,type='success',duration=2400){const out=noticeBase(message,type,duration);Promise.resolve(baseLog(type==='error'?'error':'info','UI bildirimi: '+String(message||''),{channel:'IN_APP',direction:'OUT',type})).catch(()=>{});return out;};globalThis.showAurumNotice=showAurumNotice;
const runtimeBase=setRuntime;setRuntime=function setRuntimeR225(patch){const before=currentRuntime(),next=runtimeBase(patch),changed=before.status!==next.status||before.stage!==next.stage||before.message!==next.message;if(changed)Promise.resolve(baseLog(next.status==='FAILED'?'error':'info','Sistem/Hesaplama: '+String(next.stage||'Genel')+' · '+String(next.status||''),{message:next.message||'',jobId:next.jobId||null,mode:next.mode||null})).catch(()=>{});return next;};globalThis.setRuntime=setRuntime;
globalThis.AurumNotifications=Object.freeze({version:'R225.0',open:openJournal,refresh:refreshJournal,day:dayKey});queueMicrotask(()=>{refreshJournal().catch(()=>{});installBell();try{baseLog('info','R225 süreklilik ve günlük modülü etkin',{features:['T0_LIVE_KN_MIRROR','NON_EMPTY_MARKET_DISPLAY','SOURCE_TIME_ONLY','EXACT_K_TO_S_COUNT','DAILY_NOTIFICATION_JOURNAL']})}catch{}});
})();
