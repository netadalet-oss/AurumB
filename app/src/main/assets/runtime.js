...
(function(){
  ...
  (function installR47MarketSummary(){
    ...
    globalThis.marketPage=function marketPageR47(){const cached=load();return `<div class="actions" style="margin-bottom:12px"><button class="ghost-btn" type="button" onclick="goPage('overview')">← Genel Bakışa Dön</button><button class="gold-btn" type="button" onclick="runManualData('FULL')">Veriler ile Yenile</button></div><div class="section-head"><div class="section-title"><h2>Piyasa Özeti</h2></div><small>Haber · şirket · döviz · altın · borsa</small></div><div id="aurumMarketSummaryBody">${cached?render(cached):'<div class="card"><p class="muted">Piyasa özeti yükleniyor…</p></div>'}</div>`;};
  })();
...
(function installR207FinancePortal(){
  ...
  function render(data){
    ...
    return `<div class="r207-portal-head"><div><small>BİRLEŞTİRİLMİŞ CANLI FİNANS VE GÜNDEM PORTALI</small><h2>Piyasa · Finans · Yorum · Ekonomi · Gündem · Dünya</h2><p>Öncelik: güncel ve son dakika piyasa/finans → yorum ve öneriler → ekonomi → siyaset → asayiş/adalet → dış politika → dünya → içerikli şirket/KAP. Şablon ve içeriksiz kartlar gösterilmez.</p></div><button class="ghost-btn" onclick="runManualData('FULL')">Veriler ile Yenile</button></div><div class="r207-news-chips">${cats.map(btn).join('')}</div>${lead?`<div class="r207-featured">${card(lead,true)}</div>`:'<div class="card notice">Kaynaklı ve yeterli içerik taşıyan güncel haber bulunamadı. İçeriksiz şablon kartlar gösterilmiyor.</div>'}<div class="r207-news-grid">${rest.map(x=>card(x,false)).join('')}</div><div class="r207-news-foot"><span>Kaynaklar: Google News açık RSS · Bigpara · İş Yatırım · TCMB · Borsa İstanbul</span><span>${sourceTotal?`Kaynak yanıtı ${sourceOk}/${sourceTotal} · `:''}${data?.updatedAt?'Akış yenileme: '+esc(new Date(data.updatedAt).toLocaleString('tr-TR')):''}</span></div>`
  }
...
(function installR222Contract(){
  ...
  globalThis.marketPage=function marketPageR222Unified(){
    ...
    return `<div class="actions r222-market-actions"><button class="ghost-btn" type="button" onclick="goPage('overview')">← Genel Bakışa Dön</button><button class="gold-btn" type="button" onclick="runManualData('FULL')">Veriler ile Yenile</button></div><div class="section-head r222-market-head"><div class="section-title"><h2>Piyasa Özeti</h2></div><small>Doğrudan piyasa verisi · kaynaklı haber akışı</small></div>${typeof globalThis.marketIndicatorsMarkup==='function'?globalThis.marketIndicatorsMarkup():''}<section class="r207-portal-wrap r222-unified-portal"><div id="aurumFinancePortal"><div class="card notice"><b>Birleştirilmiş finans portalı hazırlanıyor…</b><p class="muted">Piyasa, ekonomi, şirket ve küresel kaynaklar eşzamanlı taranıyor.</p></div></div></section>`;
  };
