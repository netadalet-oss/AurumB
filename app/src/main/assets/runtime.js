...
const ROW_EMPTY_THRESHOLD=60;
const COLUMN_EMPTY_THRESHOLD=60;
...
function classifyDataCompleteness(records=state.records,universe=currentSymbols(),fields=V141225_ALL_HEADERS){
  ...
  const zeroPlaceholderColumns=fields.filter(f=>f!=='Hisse'&&columnZeroCounts[f]>=Number(state.settings.columnEmptyThreshold||COLUMN_EMPTY_THRESHOLD)&&columnNonZeroNumericCounts[f]===0);
  const zeroBad=new Set(zeroPlaceholderColumns),incompleteColumns=fields.filter(f=>f!=='Hisse'&&(columnBlankCounts[f]>=Number(state.settings.columnEmptyThreshold||COLUMN_EMPTY_THRESHOLD)||zeroBad.has(f))),bad=new Set(incompleteColumns),usableFields=fields.filter(f=>f!=='Hisse'&&!bad.has(f)),rowFields=fields.filter(f=>f!=='Hisse'),bySymbol=new Map();
  ...
  for(const r of rows){let emptyCells=0;for(const f of rowFields){const missing=!v141225ValuePresent(r,f)||(zeroBad.has(f)&&v141225NumericZero(r,f));if(missing)emptyCells++;}const reasons=[];if(emptyCells>=Number(state.settings.rowEmptyThreshold||ROW_EMPTY_THRESHOLD))reasons.push(`ROW_EMPTY_CELLS_${emptyCells}`);if(r?.marketWindowEligible===false)reasons.push(`MARKET_TIME_OUTSIDE_${Math.max(30,Math.min(180,Number(state.settings?.marketFreshMinutes||90)))}M`);if(r?.jobDataStatus!=='FRESH')reasons.push('CURRENT_JOB_NOT_FRESH');bySymbol.set(r.sym,{emptyCells,eligible:reasons.length===0,reasons});}
...
function dataIntegrityGate(summary){const missingSymbols=Number(summary?.missingSymbolCount||0),missingColumns=(summary?.incompleteColumns||summary?.missingColumns||[]).length,rows=Number(summary?.loadedSymbols||0),cols=Number(summary?.fields?.length||0),fill=Number(summary?.fillPct||0),blocking=Number(summary?.activeBlockingMissingCells??summary?.emptyCells??0),minRows=Number(state.settings?.minTableRows||500),minCols=Number(state.settings?.minTableColumns||468),fillTarget=Number(state.settings?.targetMinFillPct||95),maxBlocking=Number(state.settings?.maxBlockingMissingCells||10000);const reasons=[];if(rows<minRows)reasons.push(`Satır ${rows} < ${minRows}`);if(cols<minCols)reasons.push(`Sütun ${cols} < ${minCols}`);if(blocking>maxBlocking)reasons.push(`Aktif eksik ${blocking} > ${maxBlocking}`);return {ok:reasons.length===0,missingSymbols,missingColumns,rows,cols,fillPct:fill,fillTarget,fillTargetMet:fill>=fillTarget,fillAdvisory:true,activeBlockingMissingCells:blocking,reason:reasons.join(' · ')||null};}
...
function calculationGateStatus(){const summary=dataSummary(state.records),missingColumns=(summary.incompleteColumns||summary.missingColumns||[]).length,eligibleRows=Number(summary.calculationEligibleRows||0),fillPct=Number(summary.fillPct||0),maxMissing=Math.max(0,Number(state.settings?.maxCalculationMissingColumns??50)),minRows=Math.max(1,Number(state.settings?.minCalculationRows||20)),fillOk=fillPct>=70,ok=fillOk&&missingColumns<=maxMissing&&eligibleRows>=minRows,gate={ok,missingSymbols:Number(summary.missingSymbolCount||0),missingColumns,rows:Number(summary.loadedSymbols||0),eligibleRows,fillPct,minFillPct:70,reason:ok?null:!fillOk?`Türev hesaplama kapısı: Veriler doluluğu %${fillPct.toFixed(2)} < %70; önceki Kn/K_Tarihsel/S/AL-SAT korunuyor`:`Hesaplama kapısı: eksik sütun ${missingColumns}/${maxMissing} · uygun satır ${eligibleRows}/${minRows}`};return {summary,gate,reason:gate.reason};}
...
async function r73ExtendedAudit(){const r=tableCalculationAudit(),add=(id,ok,detail,severity='error')=>{const x={id,ok:!!ok,detail,severity};r.checks.push(x);if(!x.ok)r.issues.push({id,detail,severity});};
  ...
  const sched=await r73SchedulerDiagnostic();add('SCHEDULER_END_TO_END',sched.ok,sched.detail,'warn');r.scheduler=sched;
  const activeSnapshot=(await dbGet('meta','activeDataSnapshot'))?.value||null,lastSync=(await dbGet('meta','lastSuccessfulSync'))?.value||null,selection=(await dbGet('meta','selectionSnapshot'))?.value||null;
  add('ATOMIC_DATA_SNAPSHOT',!!activeSnapshot&&Number(activeSnapshot.publishedCount||0)===Number((state.records||[]).length),activeSnapshot?`Snapshot ${activeSnapshot.snapshotId||'—'} · yayın ${Number(activeSnapshot.publishedCount||0)}/${Number((state.records||[]).length)}`:'Aktif veri snapshotı yok','warn');
  add('SUCCESS_TIMESTAMP_PERSISTENCE',!activeSnapshot||!!(lastSync||activeSnapshot.transferredAt||activeSnapshot.completedAt),lastSync?`Son başarılı veri zamanı kalıcı: ${formatTableTime(lastSync)}`:'Snapshot zamanı fallback olarak kullanılabilir','warn');
  const periodic=globalThis.AurumPeriodicMarket;add('MARKET_30M_OWNER',Number(periodic?.periodMs)===30*60*1000&&periodic?.policy==='30M_TIMER_OR_EXPLICIT_MANUAL_ONLY',`Piyasa/portal periyodu ${Number(periodic?.periodMs||0)/60000} dk · ${periodic?.policy||'sahip yok'}`);
  const mi=cachedMarketIndicators();add('MARKET_INDICATOR_CACHE',!!mi?.at&&!!mi?.values,mi?.at?`Son doğrulanmış piyasa göstergesi: ${formatTableTime(mi.at)}`:'Doğrulanmış piyasa göstergesi cache’i yok','warn');
  ...
  try{if(typeof aurumAccuracyAuditRows==='function'){const accuracy=aurumAccuracyAuditRows();for(const x of (accuracy.checks||[]))add(`ACCURACY_${x.id}`,x.ok,x.detail,x.ok?'info':'error');r.accuracy=accuracy;}}catch(e){add('ACCURACY_ENGINE',false,e?.message||String(e),'error')}
  r.summary.issues=r.issues.length;r.ok=r.issues.filter(x=>x.severity==='error').length===0;writeLocal(TABLE_AUDIT_KEY,r);r73QueueAudit(r);return r;}
function r73AuditRows(last){if(!last?.checks?.length)return '<small class="muted">Henüz denetim çalıştırılmadı.</small>';const repaired=new Set(readLocal('aurum.runtime.lastRepair.v1',{repaired:[]})?.repaired||[]);return `<div class="list">${last.checks.map(x=>`<div class="list-row"><div><b>${html(x.id)}</b><small>${html(x.detail||'')}</small></div><span class="badge ${x.ok?'ok':x.severity==='warn'?'warn':'bad'}">${x.ok?(repaired.has(x.id)?'✓ ONARILDI':'✓ ÇALIŞIYOR'):(x.severity==='warn'?'⚠ DEVAM EDİYOR':'✕ SORUN')}</span></div>`).join('')}</div>`;}
...
function tableAuditSettingsModule(){const last=readLocal(TABLE_AUDIT_KEY,null),q=r73RepairQueue();return `<details class="card gold-edge aurum-settings-details aurum-table-audit-settings"><summary class="aurum-settings-summary"><div><strong>Check-up</strong><small>Tüm uygulama · veri ve hesaplama doğruluğu · snapshot · piyasa · portal · AL/SAT · zamanlayıcı · staging · sekme hızı; yalnız teşhis eder</small></div><span class="aurum-details-chevron" aria-hidden="true">⌄</span></summary><div class="aurum-settings-details-body"><div class="list-row"><div><strong>Tüm uygulamayı yenile ve kontrol et</strong><small>Veri değiştirmez. Her yenilemede canlı sistemi tekrar sınar; açık ve düzelmiş maddeleri aşağıda gösterir.</small></div><span class="badge ${last?.ok?'ok':last?'bad':'warn'}">${last?.ok?'SON DENETİM OK':last?'BULGU VAR':'ÇALIŞTIRILMADI'}</span></div><div class="actions"><button type="button" class="gold-btn" onclick="runTableCalculationAudit()">Check-up Yenile</button></div><h3>Denetim Raporları</h3>${r73AuditRows(last)}<div class="list-row"><div><strong>Onarım için kayıt</strong><small>${q.length?`${q.length} açık bulgu Sorunları Gider / Onarım bölümüne kaydedildi.`:'Açık onarım kaydı yok.'}</small></div><span class="badge ${q.length?'bad':'ok'}">${q.length?q.length:'TEMİZ'}</span></div>${last?`<small class="muted">Son: ${html(formatTableTime(last.at))} · ${Number(last.summary?.issues||0)} bulgu</small>`:''}</div></details>`;}
...
const R221_SMART_MISSING_THRESHOLD_PCT=5,R221_SMART_MAX_ROUNDS=5,R221_PIPELINE_TARGET_MS=6*60*1000,R221_REPAIR_RESERVE_MS=75*1000;
const R225_FILL_TARGETS=Object.freeze([95,95,90,80,70]);
...
function r221CompletionMetrics(){
  ...
  return {summary,plan,missingPct,stalePct,currentPct,needs:missingPct>=R221_SMART_MISSING_THRESHOLD_PCT||stalePct>=R221_SMART_MISSING_THRESHOLD_PCT};
}
...
async function r221SmartCompletion(job,{alreadyRan=0,origin='AUTO'}={}){
  ...
  job.smartCompletion={...(job.smartCompletion||{}),origin,startedAt:job.smartCompletion?.startedAt||nowISO(),rounds,thresholdPct:R221_SMART_MISSING_THRESHOLD_PCT,maxRounds:R221_SMART_MAX_ROUNDS,fillTargets:[...R225_FILL_TARGETS]};await saveJob(job);
...
function dataQualitySettingsModule(){const s=state.settings;return aurumSettingsCard('Tablo Oluşum ve Eksiklik Eşikleri','Veri bütünlüğü · onarım · filtreler',`<div class="form-grid"><div class="field"><label>Asgari satır sayısı</label><input id="dqMinRows" type="number" min="20" value="${Number(s.minTableRows||500)}"></div><div class="field"><label>Asgari sütun sayısı</label><input id="dqMinCols" type="number" min="20" value="${Number(s.minTableColumns||468)}"></div><div class="field"><label>Hedef azami eksik hücre</label><input id="dqMaxMissing" type="number" min="0" value="${Number(s.targetMaxMissingCells||10000)}"></div><div class="field"><label>Doluluk hedefi % <small>(yayın engeli değil)</small></label><input id="dqMinFill" type="number" min="0" max="100" step="0.1" value="${Number(s.targetMinFillPct||95)}"></div><div class="field"><label>Kaynak uyuşmazlık toleransı %</label><input id="dqMismatch" type="number" min="1" max="100" step="1" value="${Number(s.sourceMismatchTolerancePct||35)}"></div><div class="field"><label>Eksik hücre toplam deneme</label><input id="dqRepairRounds" type="number" min="1" max="5" value="${Number(s.symbolRepairRounds||3)}"></div><div class="field"><label>Asgari kaynak güven puanı</label><input id="dqSourceTrust" type="number" min="0" max="100" value="${Number(s.minSourceTrustScore||35)}"></div><div class="field"><label>Hesaplamaya uygun asgari satır</label><input id="dqCalcRows" type="number" min="1" value="${Number(s.minCalculationRows||20)}"></div><div class="field"><label>Aktif bloke edici eksik sınırı</label><input id="dqBlocking" type="number" min="0" value="${Number(s.maxBlockingMissingCells||10000)}"></div><div class="field"><label>Asgari veri kalitesi</label><input id="dqQualityMin" type="number" min="0" max="100" value="${Number(s.qualityMin||55)}"></div><div class="field"><label>Satır eksik hücre eşiği</label><input id="dqRowEmpty" type="number" min="1" max="484" value="${Number(s.rowEmptyThreshold||60)}"></div><div class="field"><label>Sütun eksik hücre eşiği</label><input id="dqColumnEmpty" type="number" min="1" max="100" value="${Number(s.columnEmptyThreshold||60)}"></div><div class="field"><label>Hesaplama azami eksik sütun</label><input id="dqCalcMissingCols" type="number" min="0" max="484" value="${Number(s.maxCalculationMissingColumns??50)}"></div><div class="actions" style="grid-column:1/-1"><button class="gold-btn" onclick="saveDataQualitySettings()">Eşikleri Kaydet</button></div></div>`) }
async function saveDataQualitySettings(){try{const n=(id,lo,hi,d)=>Math.max(lo,Math.min(hi,Number($(id)?.value||d)));state.settings.minTableRows=n('#dqMinRows',20,5000,500);state.settings.minTableColumns=n('#dqMinCols',20,5000,468);state.settings.targetMaxMissingCells=n('#dqMaxMissing',0,10000000,10000);state.settings.targetMinFillPct=n('#dqMinFill',0,100,95);state.settings.sourceMismatchTolerancePct=n('#dqMismatch',1,100,35);state.settings.symbolRepairRounds=n('#dqRepairRounds',1,5,3);state.settings.minSourceTrustScore=n('#dqSourceTrust',0,100,35);state.settings.minCalculationRows=n('#dqCalcRows',1,5000,20);state.settings.maxBlockingMissingCells=n('#dqBlocking',0,10000000,10000);state.settings.qualityMin=n('#dqQualityMin',0,100,55);state.settings.rowEmptyThreshold=n('#dqRowEmpty',1,484,60);state.settings.columnEmptyThreshold=n('#dqColumnEmpty',1,100,60);state.settings.maxCalculationMissingColumns=n('#dqCalcMissingCols',0,484,50);await saveSettings();showAurumNotice('Tablo ve eksiklik eşikleri kaydedildi','success',2400)}catch(e){showAurumNotice(e?.message||String(e),'error',3600)}}
...
async function runRepairCenter(mode='DIAGNOSE'){
  try{const report=await r44RepairReport();if(mode==='DIAGNOSE'){const a=await r73ExtendedAudit();showAurumNotice(`Tanı: ${a.summary.issues} bulgu · ${report.staged} staging · ${report.jobs.length} bekleyen iş`,'info',3800);return a}
    if(mode==='AUTO_FIX'){const before=await r73ExtendedAudit(),open=new Set((before.issues||[]).map(x=>x.id));if(open.has('FAST_PARALLEL_TRANSFER')||open.has('BACKGROUND_STAGING_QUEUE'))await r73FastTransferRepair();let st={recovered:0,retained:0};if(open.has('STAGING_HEALTH')||open.has('STAGE_QUEUE_PRESSURE'))st=await r73StagingRepair();if(open.has('ADJUSTED_PRICE_INTEGRITY'))try{await repairLegacyCorruptRecordsLocal()}catch{};let sch=true;if(open.has('SCHEDULER_END_TO_END'))sch=await startScheduler();if(open.has('FAST_NAV_CACHE'))try{globalThis.AurumRawStoreR55?.invalidate?.()}catch{};if(open.has('MARKET_INDICATOR_CACHE'))try{const cached=cachedMarketIndicators();if(cached)state.marketIndicators=cached}catch{};if(open.has('FINANCE_PORTAL_CACHE'))try{await globalThis.refreshAurumFinancePortal?.(false)}catch{};for(const id of [...STAGE_BATCHES.keys()])try{await flushStageBatch(id)}catch{};const a=await r73ExtendedAudit(),remaining=new Set((a.issues||[]).map(x=>x.id)),repaired=[...open].filter(x=>!remaining.has(x));a.repair={at:nowISO(),attempted:[...open],repaired,remaining:[...remaining],stagingRecovered:st.recovered||0,scheduler:sch};writeLocal('aurum.runtime.lastRepair.v1',a.repair);showAurumNotice(`Onarım doğrulandı · ✓ ${repaired.length} düzeldi · ⚠ ${remaining.size} devam ediyor`,remaining.size?'info':'success',4800);return a}
...
function settingsPage(){return `${globalThis.AurumPresentationSettings?.markup?.()||''}${generalSettingsModule()}${performanceControlSettingsModule()}${transferArchitectureModule()}${dataTransferSettingsModule()}${dataQualitySettingsModule()}${schedulerSettingsModule()}${executionSettingsModule()}${learningSettingsModule()}${behaviorGenomeSettingsModule()}${calendarSettingsModule()}${aiApiSettingsModule()}${tableAuditSettingsModule()}${repairCenterModule()}${dataManagementModule()}${restorePointsModule()}${aurumUpdateModule()}`}
...
const MARKET_INDICATOR_META_KEY='marketIndicatorsR40';
function cachedMarketIndicators(){return state.marketIndicators||readLocal(MARKET_INDICATOR_META_KEY,null)||null;}
function marketIndicatorsMarkup(){const m=cachedMarketIndicators(),maxAge=Math.max(30,Math.min(180,Number(state.settings?.marketFreshMinutes||90)))*60000,at=Date.parse(m?.at||''),fresh=Number.isFinite(at)&&Date.now()-at<=maxAge,item=(label,key,suffix='')=>{const v=fresh?Number(m?.values?.[key]):NaN;return `<span><b>${html(label)}</b> ${Number.isFinite(v)?html(fmt(v, key==='EURUSD'?4:2)+suffix):'—'}</span>`};return `<div class="aurum-market-indicators" title="${fresh?`Kaynak zamanı ${html(m.at)}`:'Güncel ve doğrulanmış ortak piyasa göstergesi bulunmuyor'}">${item('BIST 100','XU100')}${item('USD','USDTRY')}${item('EUR','EURTRY')}${item('Parite','EURUSD')}${item('Altın gr','GRAMTRY')}${item('Ons','GOLDUSD')}</div>`;}
async function refreshMarketIndicators(){try{const symbols=['XU100.IS','TRY=X','EURTRY=X','EURUSD=X','GC=F'],url=`https://query1.finance.yahoo.com/v7/finance/quote?symbols=${encodeURIComponent(symbols.join(','))}`,res=await withProviderSlot('YAHOO_QUOTE',()=>fetchWithTimeout(url,{headers:{Accept:'application/json'},__provider:'YAHOO_QUOTE'},'Yahoo quote')),obj=await responseJSON(res),rows=obj?.quoteResponse?.result||[],by=new Map(rows.map(x=>[x.symbol,x])),num=(sym)=>validNumber(by.get(sym)?.regularMarketPrice),times=rows.map(x=>Number(x.regularMarketTime)*1000).filter(Number.isFinite),at=times.length?new Date(Math.min(...times)).toISOString():null,xu=num('XU100.IS'),usd=num('TRY=X'),eur=num('EURTRY=X'),par=num('EURUSD=X'),gold=num('GC=F'),gram=gold!=null&&usd!=null?gold*usd/31.1034768:null;if(!at)throw new Error('Piyasa gösterge zamanı doğrulanamadı');const payload={at,source:'YAHOO_QUOTE',values:{XU100:xu,USDTRY:usd,EURTRY:eur,EURUSD:par,GOLDUSD:gold,GRAMTRY:gram},provenance:{GRAMTRY:'GC=F × USDTRY / 31.1034768'}};state.marketIndicators=payload;writeLocal(MARKET_INDICATOR_META_KEY,payload);try{await dbPut('meta',{key:MARKET_INDICATOR_META_KEY,value:payload})}catch{}return payload}catch(e){return cachedMarketIndicators();}}
function dataMetaMarkup(){const summary=TABLE_META.data?.summary||dataSummary(),plan=currentPendingRepairPlan(),src=Object.entries(summary.sourceCounts||{}).sort((a,b)=>b[1]-a[1]).map(([k,v])=>`${sourceName(k)} ${v}`).join(' · ')||'Kaynak yok',mc=summary.incompleteColumns?.length??summary.missingColumns?.length??0,miss=mc?`Eksik sütun ${mc}/${summary.fields?.length||V141225_ALL_HEADERS.length}`:'Eksik sütun yok',custom=customProviders().length,builtin=9,rt=currentRuntime(),live=String(rt.stage||'')==='Veriler'&&operationBusyStatus(rt.status),stageLine=live?`<div class="aurum-staging-live"><b>Geçici doğrulama:</b> ${Number(rt.validated||0)} doğrulanmış · ${Number(rt.done||0)} denendi · ${Number(rt.failed||0)} başarısız <small>Veriler önce geçici depoda doğrulanır; ilk geçiş tamamlanınca tablo atomik yayınlanır ve eksikler ayrıca tamamlanır.</small></div>`:'';return `<div data-aurum-explicit-time>${marketIndicatorsMarkup()}${tableTimePanel('data')}<div class="aurum-data-status-card" aria-live="polite">${stageLine}<div>${summary.universeCount} hisse · <button type="button" class="aurum-inline-stat" onclick="showMissingSymbols()">${plan.symbolCount} onarım bekliyor</button> · ${Number(summary.realMissingCells??plan.missingCellCount)} gerçek eksik · ${Number(summary.exhaustedMissingCells||0)} tükenmiş · ${Number(summary.activeBlockingMissingCells||0)} aktif/bloke edici</div><div>${html(miss)} · doluluk %${Number(summary.fillPct||0).toFixed(2)} · 10.000 altı hedef ${summary.targetUnder10000?'OK':'BEKLİYOR'} · kullanılabilir ${Number(summary.usableRows||summary.loadedSymbols||0)} · hesaplamaya uygun ${Number(summary.calculationEligibleRows||0)}</div><div>${html(src)}</div><div class="aurum-source-policy">Kalite öncelikli paralel işçiler · sağlayıcı bazlı hız sınırı · erişilemeyen hisse pas geçilir, önceki veri korunur · ${builtin} yerleşik uç nokta${custom?` + ${custom} özel`:''} · “Eksikleri Tamamla” = Akıllı Tamamlama; ≤30 dk yalnız eksikleri, >30 dk tüm tabloyu yeniden doğrular<small class="aurum-fill-policy">Doluluk hedefi önce %95; sağlanmazsa kademeli ek denemeler %90 → %80 → son güvenlik eşiği %70. %70 altı Veriler görünür kalır fakat Kn, K_Tarihsel, S ve AL/SAT önceki geçerli snapshot ve zaman damgasını korur.</small></div></div></div>`;}
function dataPage(){
  ...
  const content=blocked
    ? `<div class="card notice aurum-integrity-block"><b>Veriler tablosu doğrulama eşiğini geçemedi</b><p>${html(integrity.reason)}</p><small>Tablo atomik olarak yayınlanır; ancak türev hesaplamalar kalite koşulları sağlanana kadar bekler. “Eksikleri Tamamla” alternatif kaynaklarla seçici onarım yapar.</small></div>`
    : `${v141225Tools()}<div id="recordsContent" class="aurum-stable-table" data-aurum-explicit-time>${v141225Table(rows)}</div>`;
...
function integrityBlockedMarkup(label){const x=calculationGateStatus();return x.gate.ok?'':`<div class="card notice aurum-integrity-block"><b>${html(label)} oluşturulamaz</b><p>${html(x.reason)}</p><small>Mevcut doğrulanmış Veriler tablosu düzeltilmeden bu tablo hesaplamaya veya yeni sonuca açılmaz.</small></div>`;}
...
(function(){
  ...
  calculateS=async function calculateSR47(job){
    ...
    try{
      await pauseCheckpoint(job,JOB_STATUS.S_RUNNING);const calcRecords=calculationRecords();if(calcRecords.length<20)throw new Error(`S için en az 20 doğrulanmış hisse gerekli; ${calcRecords.length} uygun hisse var`);
      ...
      if((built.selection||[]).length<20)throw new Error(`S için 20 hisse üretilemedi; model ${built.selection?.length||0} uygun seçim üretti`);
      state.scores=built.scores;state.selection=built.selection;state.modelBySym=built.bySym;await updateSelectionLifecycle();
...
(function(){
  ...
  const pageFns=()=>({overview,market:globalThis.marketPage||overview,selection:selectionPage,data:function r61DataPage(){
    const rows=currentV141225Rows(),buttons=`<button class="gold-btn" onclick="AurumRuntime.manualData('GENERAL')" ${S.syncing||S.calculating?'disabled':''}>Verileri Güncelle</button>`,integrity=calculationGateStatus(),blocked=!integrity.gate.ok&&S.records.length>0,body=blocked?`<div class="card notice aurum-integrity-block"><b>Veriler tablosu doğrulama eşiğini geçemedi</b><p>${html(integrity.reason)}</p><small>Tablo atomik olarak yayınlanır; ancak türev hesaplamalar kalite koşulları sağlanana kadar bekler. “Eksikleri Tamamla” alternatif kaynaklarla seçici onarım yapar.</small></div>`:`${v141225Tools()}<div id="recordsContent" class="aurum-stable-table" data-aurum-explicit-time>${r61DataSkeleton(rows)}</div>`;return `${operationBlock('data',buttons)}${dataMetaMarkup()}${body}`;
...
(function(){
  try{
    ...
    if(localStorage.getItem(KEY)!=='1'){
      ...
      if(st?.settings){
        st.settings.rowEmptyThreshold=60;
        st.settings.columnEmptyThreshold=60;
  ...
  prepareGeneralData=async function prepareGeneralDataR23(job,mode='GENERAL'){
    ...
    try{
      ...
      const startup=await Promise.allSettled([
        ...
        Promise.resolve(cachedMarketIndicators()),
...
(function installR33DeterministicS(){
  ...
  calculateS=async function calculateSR33(job){
    ...
    try{
      ...
      if(calcRecords.length<20)throw new Error(`S için en az 20 doğrulanmış hisse gerekli; ${calcRecords.length} uygun hisse var`);
...
(function installAurumR2SettingsUX(){
  ...
  function enhance(){
    ...
    document.querySelectorAll('input[type="range"][id^="aurumScaleVal_"]').forEach(r=>{
      ...
      r.min='90';r.max='110';r.step='2';r.value=String(Math.max(90,Math.min(110,Number(r.value)||100)));const preview=()=>{ if(title){const v=Math.max(90,Math.min(110,Number(r.value)||100)); const base=parseFloat(title.dataset.r2BaseSize)||14; title.style.fontSize=(base*v/100)+'px';} };
...
(()=>{
  ...
  async function applyCompletenessDefaults(){
    state.settings.targetMinFillPct=TARGET_FILL;
    state.settings.targetMaxMissingCells=Math.floor(currentSymbols().length*V141225_ALL_HEADERS.length*(1-TARGET_FILL/100));
    state.settings.maxBlockingMissingCells=Math.min(Number(state.settings.maxBlockingMissingCells||999999),state.settings.targetMaxMissingCells);
...
(function installR207FinancePortal(){
  ...
  globalThis.AurumNewsPortal=Object.freeze({filter:c=>{active=String(c||'ALL');paint(load())},refresh:globalThis.refreshAurumFinancePortal,renderCached:()=>{const c=load();return c?render(c):''},repaintCached:()=>paint(load())});
...
`;document.head.appendChild(style);
  ...
  if(typeof baseMarketPage==='function')globalThis.marketPage=function marketPageR214WithPortal(){
    const c=load();
    setTimeout(()=>{try{paint(load())}catch{}},0);
...
(()=>{
  ...
  queueMicrotask(()=>{try{if(!Number.isFinite(Number(state.settings.targetMinFillPct))||Number(state.settings.targetMinFillPct)===96)state.settings.targetMinFillPct=95;saveSettings?.()}catch{}try{AurumUpdateAPI.state.r209={version:'REV20.9-MARKET-UX-ADVISORY-FILL',activatedAt:nowISO(),features:['MARKET_STRIP_MANUAL_REFRESH','MARKET_STRIP_AUTO_REFRESH_AFTER_DATA_UPDATE','FILL_95_IS_TARGET_NOT_PUBLISH_GATE','30M_COHORT_GATE_PRESERVED']}}catch{}});
})();
...
(function installR233UnifiedDirectMarketFeed(){
 ...
 function markup(){
   const m=cached()||{},f=m.fields||{},rows=KEYS.map(k=>{const x=f[k]||{},p=Number(x.changePct),ok=Number.isFinite(p),cls=!ok?'flat':p>0?'up':p<0?'down':'flat',arrow=!ok?'':p>0?'↑':p<0?'↓':'·',pct=!ok?(x.stale?'önceki veri':'—'):(p>0?'+':'')+p.toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2})+'%',src=String(x.source||'kaynak bekleniyor').replace('_DIRECT','').replaceAll('_',' ');
...
(()=>{
  ...
  async function refresh(){
    ...
    const old=cached()?.fields||{};
    for(const k of KEYS){
      const fresh=fields[k],prior=old[k],freshComplete=fresh?.value!=null&&Number.isFinite(Number(fresh?.changePct)),priorComplete=prior?.value!=null&&Number.isFinite(Number(prior?.changePct));
      if(!fresh&&prior)fields[k]={...prior,stale:true};
      else if(!freshComplete&&priorComplete)fields[k]={...prior,stale:true};
    }
