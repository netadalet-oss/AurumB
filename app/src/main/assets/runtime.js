...
function tableAuditSettingsModule(){const last=readLocal(TABLE_AUDIT_KEY,null),q=r73RepairQueue(),accuracy=readLocal('aurumAccuracyUiLast',null);return `<details class="card gold-edge aurum-settings-details aurum-table-audit-settings"><summary class="aurum-settings-summary"><div><strong>Check-up</strong><small>Tüm uygulama · veri ve hesaplama doğruluğu · snapshot · piyasa · portal · AL/SAT · zamanlayıcı · staging · sekme hızı; yalnız teşhis eder</small></div><span class="aurum-details-chevron" aria-hidden="true">⌄</span></summary><div class="aurum-settings-details-body"><div class="list-row"><div><strong>Tüm uygulamayı yenile ve kontrol et</strong><small>Veri değiştirmez. Her yenilemede canlı sistemi tekrar sınar; açık ve düzelmiş maddeleri aşağıda gösterir.</small></div><span class="badge ${last?.ok?'ok':last?'bad':'warn'}">${last?.ok?'SON DENETİM OK':last?'BULGU VAR':'ÇALIŞTIRILMADI'}</span></div><div class="actions"><button type="button" class="gold-btn" onclick="runTableCalculationAudit()">Check-up Yenile</button><button type="button" class="ghost-btn" onclick="aurumRunAccuracyAudit(false).then(r=>{writeLocal('aurumAccuracyUiLast',r);renderCurrentPagePreservingView()})">Veri / Hesaplama Doğruluğunu Denetle</button></div><h3>Denetim Raporları</h3>${r73AuditRows(last)}<div class="list-row"><div><strong>Veri ve hesaplama doğruluğu</strong><small>Veriler bütünlük kapısı · BIST oturumları · K_Tarihsel d+1 · ReelTop20 · Kn referansları · kaynak zamanı ve uyuşmazlıkları</small></div><span class="badge ${accuracy?.ok?'ok':accuracy?'bad':'warn'}">${accuracy?.ok?'✓ ÇALIŞIYOR':accuracy?'✕ BULGU VAR':'ÇALIŞTIRILMADI'}</span></div><div class="list-row"><div><strong>Onarım için kayıt</strong><small>${q.length?`${q.length} açık bulgu Sorunları Gider / Onarım bölümüne kaydedildi.`:'Açık onarım kaydı yok.'}</small></div><span class="badge ${q.length?'bad':'ok'}">${q.length?q.length:'TEMİZ'}</span></div>${last?`<small class="muted">Son: ${html(formatTableTime(last.at))} · ${Number(last.summary?.issues||0)} bulgu</small>`:''}</div></details>`;}
