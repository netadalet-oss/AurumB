...
function visiblePrice(rec){const live=Number(rec?.livePrice);if(Number.isFinite(live)&&live>0)return live;const held=Number(rec?.lastVisiblePrice);if(Number.isFinite(held)&&held>0)return held;return latestClose(rec);}
