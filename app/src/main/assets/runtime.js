...
function calculationGateStatus(){const summary=dataSummary(state.records),missingColumns=(summary.incompleteColumns||summary.missingColumns||[]).length,eligibleRows=Number(summary.calculationEligibleRows||0),maxMissing=Math.max(0,Number(state.settings?.maxCalculationMissingColumns??50)),minRows=Math.max(1,Number(state.settings?.minCalculationRows||20)),ok=missingColumns<=maxMissing&&eligibleRows>=minRows,gate={ok,missingSymbols:Number(summary.missingSymbolCount||0),missingColumns,rows:Number(summary.loadedSymbols||0),eligibleRows,reason:ok?null:`Hesaplama kapısı: eksik sütun ${missingColumns}/${maxMissing} · uygun satır ${eligibleRows}/${minRows}`};return {summary,gate,reason:gate.reason};}
...
async function prepareMissingData(job){
  ...
  try{
    ...
    const worker=async()=>{while(true){await pauseCheckpoint(job,JOB_STATUS.FETCHING_DATA);const idx=cursor++;if(idx>=targets.length)break;const sym=targets[idx];if(__resumeFresh.has(sym))continue;const prior=state.recordMap.get(sym)||null,base=prior?.series?.date?.length?bundleFromRecord(prior):null,beforeMissing=prior?bundleRecordMissingFields(prior):['*'];let rec=prior?JSON.parse(JSON.stringify(prior)):makePlaceholder(sym,null,['REPAIR_TARGET_NO_PRIOR']);
      ...
      await stagePut(job.id,sym,rec);completed++;job.processedSymbols=completed;setRuntime({status:JOB_STATUS.FETCHING_DATA,jobId:job.id,mode:job.mode,stage:'Veriler',done:completed,total:targets.length,message:`${sym} · onarım denendi`,symbol:sym,provider:null});if(completed%32===0)await saveJob(job);
    }};
...
const R221_SMART_MISSING_THRESHOLD_PCT=5,R221_SMART_MAX_ROUNDS=3,R221_PIPELINE_TARGET_MS=6*60*1000,R221_REPAIR_RESERVE_MS=75*1000;
...
async function r221SmartCompletion(job,{alreadyRan=0,origin='AUTO'}={}){
  ...
  job.smartCompletion={...(job.smartCompletion||{}),origin,startedAt:job.smartCompletion?.startedAt||nowISO(),rounds,thresholdPct:R221_SMART_MISSING_THRESHOLD_PCT,maxRounds:R221_SMART_MAX_ROUNDS};await saveJob(job);
  while(rounds<R221_SMART_MAX_ROUNDS&&(m.needs||((origin==='MANUAL_MAIN'||origin==='MANUAL_REPAIR_FULL')&&rounds===0&&m.plan.symbolCount>0))&&!cancelRequested(job)){
    const before={fillPct:m.summary.fillPct,missingPct:m.missingPct,stalePct:m.stalePct,symbolCount:m.plan.symbolCount};
    rounds++;setRuntime({status:JOB_STATUS.FETCHING_DATA,jobId:job.id,mode:job.mode,stage:'Veriler',done:0,total:m.plan.symbolCount,message:`Akıllı tamamlama ${rounds}/${R221_SMART_MAX_ROUNDS} · eksik %${m.missingPct.toFixed(2)} · güncel olmayan %${m.stalePct.toFixed(2)}`});
    const ok=await prepareMissingData(job);m=r221CompletionMetrics();job.smartCompletion={...(job.smartCompletion||{}),rounds,lastAt:nowISO(),lastBefore:before,lastAfter:{fillPct:m.summary.fillPct,missingPct:m.missingPct,stalePct:m.stalePct,symbolCount:m.plan.symbolCount},lastOk:ok};await saveJob(job);
    if(!ok){job.smartCompletion.stopped='REPAIR_FAILED_LAST_VALID_TABLE_PRESERVED';try{await transition(job,JOB_STATUS.DATA_COMPLETED,{error:null,message:`İlk doğrulanmış tablo korundu · akıllı tamamlama ${rounds}. turda durdu`})}catch{}break}
  }
  job.smartCompletion={...(job.smartCompletion||{}),completedAt:nowISO(),rounds,finalFillPct:Number(m.summary.fillPct||0),finalMissingPct:m.missingPct,finalStalePct:m.stalePct,remainingSymbols:m.plan.symbolCount,completed:!m.needs};await saveJob(job);return m;
}
async function runAutoJob(job,mode='GENERAL'){
  ...
  const gate=calculationGateStatus();if(!gate.gate.ok){await transition(job,JOB_STATUS.FAILED,{error:'POST_REPAIR_CALCULATION_GATE',message:`Veriler yayınlandı fakat türev tablolar için kalite kapısı geçilmedi · ${gate.reason||''}`});return false}
...
async function resumeAutoJob(job,mode='GENERAL'){
  ...
  if(['DATA_COMPLETED','KN_RUNNING'].includes(status)){if(!sameSnapshot)return runAutoJob(job,mode);r221StartBudget(job);if(!job?.smartCompletion?.completedAt)await r221SmartCompletion(job,{origin:'AUTO_RESUME'});if(!(await calculateKn(job)))return false;if(!(await archiveHistorical(job)))return false;return calculateS(job)}
...
async function runManualData(mode='GENERAL'){
  ...
  showAurumNotice(`Veriler yayınlandı · doluluk %${Number(m.summary.fillPct||0).toFixed(2)} · eksik %${m.missingPct.toFixed(2)} · akıllı tamamlama ${Number(job.smartCompletion?.rounds||0)}/${R221_SMART_MAX_ROUNDS}`,'success',4600);return true
}
...
(function(){
  ...
  function r47T0FinalStatus(){
    ...
    const missing=syms.filter(sym=>!r47RecordClosed(state.recordMap.get(sym),t0.date));return {ready:missing.length===0,t0,syms,missing};
  }
...
(()=>{
  ...
  function workerCount(total){const net=String(navigator?.connection?.effectiveType||''),hc=Math.max(4,Number(navigator?.hardwareConcurrency||8));let cap=Math.max(48,Math.min(72,Number(state.settings?.concurrency||56),Math.max(48,hc*7)));if(/2g/.test(net))cap=Math.min(cap,6);else if(/3g/.test(net))cap=Math.min(cap,20);return Math.max(1,Math.min(total,cap));}
...
(()=>{
  ...
  function workerCount(total,rescue=false){const net=String(navigator?.connection?.effectiveType||''),hc=Math.max(4,Number(navigator?.hardwareConcurrency||8)),bg=!!document.hidden;let n;if(rescue)n=bg?Math.max(20,Math.min(32,hc*4)):Math.max(14,Math.min(24,hc*3));else n=bg?Math.max(32,Math.min(44,hc*5)):Math.max(28,Math.min(38,hc*4));if(/2g/.test(net))n=Math.min(n,6);else if(/3g/.test(net))n=Math.min(n,16);return Math.max(1,Math.min(total,n))}
