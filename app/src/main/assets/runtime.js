...
function dataPage(){
  ...
  const warning=blocked?`<div class="card notice aurum-integrity-block"><b>Yeni türev hesaplama kalite eşiğinde bekliyor</b><p>${html(integrity.reason)}</p><small>Mevcut doğrulanmış Veriler tablosu görünür ve korunur; kalite kapısı yalnız yeni Kn/K_Tarihsel/S üretimini engeller. “Eksikleri Tamamla” alternatif kaynaklarla seçici onarım yapar.</small></div>`:'';
...
function integrityBlockedMarkup(label){const x=calculationGateStatus();return x.gate.ok?'':`<div class="card notice aurum-integrity-block"><b>${html(label)} için yeni hesaplama bekliyor</b><p>${html(x.reason)}</p><small>Kalite kapısı yalnız yeni hesap üretimini durdurur; son doğrulanmış tablo/snapshot aşağıda korunur ve gösterilir.</small></div>`;}
