...
function dataIntegrityGate(summary){const fill=Number(summary?.fillPct||0);return {ok:true,missingSymbols:Number(summary?.missingSymbolCount||0),missingColumns:(summary?.incompleteColumns||summary?.missingColumns||[]).length,rows:Number(summary?.loadedSymbols||0),cols:Number(summary?.fields?.length||0),fillPct:fill,reason:null};}
