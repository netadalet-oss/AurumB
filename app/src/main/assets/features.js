...
(()=>{
  ...
  globalThis.marketPage=function(){const cached=fmLoad();setTimeout(()=>globalThis.refreshAurumMarketSummary?.(false),0);return `<div class="actions" style="margin-bottom:12px"><button class="ghost-btn" type="button" onclick="goPage('overview')">← Genel Bakışa Dön</button><button class="gold-btn" type="button" onclick="refreshAurumMarketSummary(true)">Yenile</button></div><div class="section-head"><div class="section-title"><h2>Piyasa Özeti</h2></div><small>Haber · öneri · fon · döviz · altın</small></div><div id="aurumMarketSummaryBody">${cached?fmRenderData(cached):'<div class="card"><p class="muted">Piyasa özeti yükleniyor…</p></div>'}</div>`};
...
(()=>{
  ...
  setTimeout(()=>{try{resolveRewardEvents(state?.records||[])}catch{}},0);
})();
