...
(()=>{
 ...
 async function health(){
  const fill=activeFill(),meta=await globalThis.dbGet?.('meta','activeDataSnapshot'),rt=globalThis.AurumRuntime?.status?.()||{};
  const report={at:iso(),fillPct:fill,snapshotAt:meta?.value?.changedAt||meta?.value?.transferredAt||null,runtime:rt.status||'IDLE',scheduler:await globalThis.r73SchedulerDiagnostic?.().catch?.(()=>null),market:globalThis.cachedMarketIndicators?.()?.updatedAt||null};
